			       ...
			       struct snd_sof_dai *dai)
{
	...
	ret = sof_parse_topology_tokens(scomp, comp_dai, SOF_TOKEN_DAI, private->array,
				    ...
				    struct snd_soc_tplg_dapm_widget *tw)
{
	...
	ret = sof_parse_topology_tokens(scomp, pipeline, SOF_TOKEN_SCHED, private->array,
					le32_to_cpu(private->size));
	if (ret != 0)
		return ret;
			    ...
			    struct snd_soc_tplg_dapm_widget *tw)
{
	...
	ret = sof_parse_topology_tokens(scomp, &swidget->comp_ext, SOF_TOKEN_EXT, tw->priv.array,
					le32_to_cpu(tw->priv.size));
	if (ret != 0) {
		kfree(swidget);
		return ret;
			 ...
			 struct snd_soc_tplg_link_config *cfg)
{
	...
	ret = sof_parse_topology_tokens(scomp, &common_config, SOF_TOKEN_DAI_LINK, private->array,
					le32_to_cpu(private->size));
	if (ret != 0)
		return ret;
