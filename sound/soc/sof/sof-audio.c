...
/* update DAI config for HDA/ALH type static DAI widgets both during widget setup and free */
static int sof_static_widget_config(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget)
{
	...
	ret = sof_dai_config_setup(sdev, dai, NULL);
	if (ret < 0)
...
static int sof_widget_free(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget)
{
	...
	if (swidget->use_count > 1) {
		if (!swidget->dynamic_pipeline_widget)
			ret = sof_static_widget_config(sdev, swidget);
...
int sof_widget_list_free(struct snd_sof_dev *sdev, struct snd_sof_pcm *spcm, int dir)
{
	...
	for_each_dapm_widgets(list, i, widget) {
		...
		/* free widget and its pipe_widget */
