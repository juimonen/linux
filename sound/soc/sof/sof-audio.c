...
sof_prepare_widgets_in_path(struct snd_sof_dev *sdev, struct snd_soc_dapm_widget *widget,
			    ...
			    struct snd_pcm_hw_params *runtime_params,
			    struct snd_pcm_hw_params *input_params, int dir)
{
	const struct sof_ipc_tplg_ops *ipc_tplg_ops = sdev->ipc->ops->tplg;
	...
	ret = widget_ops[widget->id].prepare(swidget, platform_params, runtime_params,
					     input_params, dir);
	if (ret < 0) {
	...
	snd_soc_dapm_widget_for_each_sink_path(widget, p) {
		if (!p->walking && p->sink->dobj.private) {
			p->walking = true;
			ret = sof_prepare_widgets_in_path(sdev, p->sink, platform_params,
							  runtime_params, input_params, dir);
			p->walking = false;
			if (ret < 0)
				return ret;
		}
...
sof_widget_op_in_order(struct snd_sof_dev *sdev, struct snd_soc_dapm_widget_list *list,
		       struct snd_sof_platform_stream_params *platform_params,
		       struct snd_pcm_hw_params *runtime_params, int dir,
		       enum sof_widget_op op)
{
	...
	for_each_dapm_widgets(list, i, widget) {
		...
		{
			struct snd_pcm_hw_params *input_params;
			 ...
			 */
			input_params = kmemdup(runtime_params, sizeof(*runtime_params),
					       GFP_KERNEL);
			if (!input_params)
				return -ENOMEM;
			ret = sof_prepare_widgets_in_path(sdev, widget, platform_params,
							  runtime_params, input_params, dir);
			kfree(input_params);
			str = "prepare";
			break;
			  ...
			  struct snd_pcm_hw_params *runtime_params, int dir)
{
	...
	ret = sof_widget_op_in_order(sdev, list, platform_params, runtime_params,
...
widget_free:
	sof_widget_op_in_order(sdev, list, platform_params, runtime_params, dir, SOF_WIDGET_FREE);
...
int sof_widget_list_free(struct snd_sof_dev *sdev, struct snd_sof_pcm *spcm, int dir)
{
	...
	ret = sof_widget_op_in_order(sdev, list, NULL, NULL, dir, SOF_WIDGET_FREE);
