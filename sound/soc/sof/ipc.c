...
static void sof_set_get_mmio(struct snd_sof_dev *sdev,
...
static int sof_get_ctrl_size_params(struct snd_sof_control *scontrol,
				    struct sof_ipc_ctrl_data *cdata,
				    struct sof_ipc_ctrl_data_params *sparams)
{
	/*
	 * Calculate the ctrl data offsets and sizes, because we have a union
	 * as the last member and the scontrol->size doesn't reflect the actual
	 * payload size, but rather the maximum.
	 */
				    ...
				    struct sof_ipc_ctrl_data_params *sparams)
{
	...
	case SOF_CTRL_TYPE_VALUE_CHAN_GET:
	case SOF_CTRL_TYPE_VALUE_CHAN_SET:
		sparams->src = (unsigned char *)src->chanv;
		sparams->dst = (unsigned char *)dst->chanv;
		break;
	case SOF_CTRL_TYPE_VALUE_COMP_GET:
	case SOF_CTRL_TYPE_VALUE_COMP_SET:
		sparams->src = (unsigned char *)src->compv;
		sparams->dst = (unsigned char *)dst->compv;
		break;
	case SOF_CTRL_TYPE_DATA_GET:
	case SOF_CTRL_TYPE_DATA_SET:
		sparams->src = (unsigned char *)src->data->data;
		sparams->dst = (unsigned char *)dst->data->data;
		break;
	default:
		return -EINVAL;
	}
				       ...
				       int send)
{
	...
	partdata = (struct sof_ipc_ctrl_data *)
				  ...
				  enum sof_ipc_ctrl_cmd ctrl_cmd)
{
	...
	if (scontrol->readback_offset != 0) {
		/* write/read value header via mmio region */
		sof_set_get_mmio(sdev, scontrol, cdata, send);
		return 0;
	}
	...
	sof_get_ctrl_size_params(scontrol, cdata, &sparams);
	...
	if (v->abi_version < SOF_ABI_VER(3, 2, 0)) {
