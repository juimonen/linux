...
struct snd_sof_widget {
	...
	int complete;
	int use_count;
	struct mutex use_count_mutex; /* protect widget use_count */
	int core;
	int id;

	/*
	 * Flag indicating if the widget should be set up dynamically when a PCM is opened.
	 * If not set, then the widget is set up at the time of topology loading and retained
	 * until the DSP enters D3. It will need to set up again when resuming from D3.
	 */
	int dynamic_pipeline_widget;
